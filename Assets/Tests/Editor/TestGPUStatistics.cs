...
namespace GPUImageStatisticsSystem.Editor {
	...
	public class TestGPUStatistics {
		...
		[Test]
		public void GPUStatisticsLinearColorSpace() {
			RunGPUStatistics(StatisticsColorSpace.Linear);
		}

		[Test]
		public void GPUStatisticsSRGBColorSpace() {
			RunGPUStatistics(StatisticsColorSpace.SRGB);
		}

		void RunGPUStatistics(StatisticsColorSpace colorSpace) {
			var tex = Resources.Load<Texture2D>(IMAGE_NAME);
			var pixels = ExpectedPixels(tex, colorSpace);
			var expectedSum = pixels.Total();
			var expectedAverage = pixels.Average();
			var expectedCovariance = pixels.Covariance(expectedAverage);

			var stat = new GPUStatistics { ColorSpace = colorSpace };
		...
		static System.Collections.Generic.IEnumerable<Vector4> ExpectedPixels(
				Texture2D tex, StatisticsColorSpace colorSpace) {
			var colors = tex.GetPixels();
			if (PlayerSettings.colorSpace == ColorSpace.Linear) {
				if (colorSpace == StatisticsColorSpace.SRGB)
					return colors.Select(c => (Vector4)c.gamma);
				return colors.Select(c => (Vector4)c);
			}
			return colors.ColorToVector4();
		}
